...
const GymAttendance = () => {
  ...
  useEffect(() => {
    if (gymId) {
      axios
        .get(`http://localhost:5000/api/attendance/${gymId}`)
        .then((response) => {
          setAttendance(
  ...
  const handleCheckIn = async () => {
    try {
      const response = await axios.post("http://localhost:5000/api/attendance/checkin", { gymId });
  ...
  const handleCheckOut = async (attendanceId) => {
    try {
      const response = await axios.post("http://localhost:5000/api/attendance/checkout", { gymId });
      setAttendance(
        attendance.map((att) =>
